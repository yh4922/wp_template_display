...
type DatasetVersionDataBatchEmptyReq struct {
	Ids []uint32 `json:"ids" validate:"required"`
}

func DatasetVersionDataBatchEmpty(c *fiber.Ctx) error {
	// 模型名称
	c.Locals("Action", "/api/v1/dataset/version/data/batch-empty")
	...
	// 设置图片
	err = m.BatchSetVersionImageToEmpty(version, verImages, UserId)
	if err != nil {
